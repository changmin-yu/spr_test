...
class SPRCatDqnModel(torch.nn.Module):
    ...
    def __init__(
        ...
        #                                            n_atoms=n_atoms,
        #                                            std_init=noisy_nets_std)
        self.head = MujocoFfModel((self.hidden_size, ), 
                                  output_size, 
                                  hidden_sizes=[self.dqn_hidden_size]*2, 
                                  init_log_std=noisy_nets_std)
    ...
    def forward(self, observation,
        ...
        if train:
            ...
            if self.renormalize:
                conv_out = renormalize(conv_out, -3)
            mu, std, p = self.head(conv_out)
            
            pred_action_dist = MultivariateNormal(loc=mu, covariance_matrix=std.exp())
            return pred_action_dist, p
    ...
    def select_action(self, obs, noisy=False):
        pred_action_dist, value = self.forward(obs, None, None, train=False, eval=True)

        # if self.distributional:
        #     value = from_categorical(value, logits=False, limit=10)
        # return value
        if noisy:   
            return pred_action_dist.sample(), value
        return pred_action_dist.mean, value
