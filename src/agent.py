...
class SPRAgent(AtariCatDqnAgent):
    ...
    def initialize(self,
        ...
        self.search = SPRActionSelection(self.model, self.noisy)
...
class SPRActionSelection(torch.nn.Module):
    def __init__(self, network, noisy=True, device="cpu"):
        super().__init__()
    ...
    def run(self, obs):
        ...
        action, value = self.network.select_action(obs, self.noisy)
