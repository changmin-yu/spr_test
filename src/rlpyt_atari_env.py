...
print('pay attention to mujoco_gl')
os.environ['MUJOCO_GL'] = 'osmesa'
print('mujoco_gl environment set to egl')

EnvInfo = namedtuple("EnvInfo", ["game_score", "traj_done"])
dmControlEnvInfo = namedtuple("EnvInfo", ["discount"])
...
class dmControlTrajInfo(TrajInfo):
    """TrajInfo class for use with Atari Env, to store raw game score separate
    from clipped reward signal."""
    ...
    def step(self, observation, action, reward, done):
        super().step(observation, action, reward, done)
        # self.GameScore += getattr(env_info, "game_score", 0)


class DeepMindControlEnv(Env):
    def __init__(self, game='walker_walk', 
                 imagesize=64, 
                 seed=0, 
                 id=0, 
                 grayscale=False, 
        ...
        print('loaded suite environment')
        self._action_spec = self._env.action_spec()
        self._action_space = FloatBox(low=self._action_spec.minimum[0], high=self._action_spec.maximum[0], shape=self._action_spec.shape)
        self.channels = 1 if grayscale else 3
        self.grayscale = grayscale
        self.imagesize = imagesize
        obs_shape = (imagesize, imagesize, self.channels)
    ...
    def step(self, action):
        ...
        done = time_step.last() or self._step_counter >= self._horizon
        info = dmControlEnvInfo(discount=np.array(time_step.discount, dtype=np.float32))
        self._step_counter += 1
    ...
    def render(self, size=None, *args, **kwargs):
        ...
        if 'camera' not in kwargs:
            camera = dict(quadruped=2).get(self._domain, 0)
        else:
            camera = kwargs['camera']
        if size is None:
            return self._env.physics.render(*self.obs_shape[:-1], camera_id=camera)
        return self._env.physics.render(*size, cemra_id=camera)
